...
replace vcvg = 0.473684210526316 in 1
replace vcvg = 0.526315789473684 in 2
replace vcvg = 0.578947368421053 in 3
replace vcvg = 0.631578947368421 in 4
replace vcvg = 0.684210526315789 in 5
replace vcvg = 0.736842105263158 in 6
replace vcvg = 0.789473684210526 in 7
replace vcvg = 0.842105263157895 in 8
replace vcvg = 0.894736842105263 in 9
replace vcvg = 0.947368421052632 in 10
...
local plotit  graph twoway ///
  ...
  , legend( off) ///
	 ...
	 xline(40 60, lcolor(gs12) lstyle(foreground))
	 
`plotit'

*graph export coverage_50_8_ess_values.png, width(2000) replace
...
local plotit  graph twoway ///
	...
	(rspike mfudgelo mfudgehi y , sort horizontal lcolor(white) lwidth(*0.5)) /// // cover much of the rcap line
		...
		`spike3' ///
		`spike4' ///
 	(scatter y x, mlabel(cistring) m(i) mlabsize(*.65) mlabcolor(gs10) xscale(range(35 80)) ) ///
  , legend( off) ///
     ysize(15) xsize(15) ///
	 ylabel(1(1)8, ang(hor) nogrid valuelabel labsize(small)) graphregion(color(white)) ///	 
	 xscale(titlegap(*10)) yscale(titlegap(1)) ///
	 ...
	 xline(80, lcolor(black) lstyle(foreground)) ///
	 xline(95,  lcolor(red)   lstyle(foreground)) 
